import time
from z3 import Optimize, String, Or, Implies, And, set_param, Solver, unsat, sat, Sum, If, Bool, IntVal, AtMost, Not
...
def generate_objective_smt_expression(direct_dependencies, transitive_dependencies, ctx, add_soft_clauses):
    """
    Generate an SMT expression using Boolean variables for version selection,
    with hard constraints for compatibility and soft clauses for version preferences.
    ...
    # Declare Boolean variables for each package-version pair
    version_selected = {}
    for pkg in all_packages:
    ...
    for package, versions in direct_dependencies.items():
        if versions:
            constraints.append(Or([version_selected[package][v] for v in versions]))
    ...
    for package_version, dependencies in transitive_dependencies.items():
        if isinstance(dependencies, dict):
            package, version = package_version.split("==")
            for dep_package, dep_versions in dependencies.items():
                dep_constraint = Or([version_selected[dep_package][v] for v in dep_versions])
                constraints.append(Implies(version_selected[package][version], dep_constraint))
    ...
    if add_soft_clauses:
        for package, versions in package_versions.items():
            if versions:
                num_versions = len(versions)
                # Soft clause to prefer not installing the package
                not_installed = And([Not(version_selected[package][v]) for v in versions])
                solver.add_soft(not_installed, 1)
                # Soft clauses to prefer newer versions
                for i, version in enumerate(versions):
                    weight = i / num_versions   # Normalized weight, increasing with recency
                    solver.add_soft(version_selected[package][version], weight)
